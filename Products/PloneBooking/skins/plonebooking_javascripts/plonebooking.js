...
function getPeriodicityQuery(periodicity_type, periodicity_end_date, periodicity_variable) {
    ...
    if (periodicity_variable != null) {
       if (periodicity_type_value == "4") {
           query = query + '&periodicity_variable=';
       } else {       
...
Timeline = function(content, start_ts, length_ts, booking_interval) {
    ...
    function onmouseup(e) {
        ...
        sendAjaxRequest(cursor.href + '/testBookingPeriod?start_ts='+start_ts+'&end_ts='+end_ts,
            function(xhReq) {
             // alert(isHTML(xhReq.responseText));  // should return true
             // alert(isHTML("Phone number")); // should return false
             if (isHTML(xhReq.responseText) == true) {
                 var retvalue = $(xhReq.responseText).text().trim();
             } else { 
                 var retvalue = xhReq.responseText
             }            
                // get from html DOC the string True from element p like <p>True</p> and strip all other characters like carriage returns
             //   var retvalue = $(xhReq.responseText).text().trim();
                 //alert(retvalue);
                if (retvalue != "True") {
                    if (retvalue.split('\n').length > 1) {
                       
                         alert(retvalue.substring(retvalue.indexOf('\n') + 1)); // here we jump over the second line which contains the booking start and end time like 1464795000:1464795000
                         cursor = null;
                         return false;
                        } 
...
Booking.editPopup = function (e) {
    // alert(Booking.form.isAnon.value)
    if (Booking.form.isAnon.value == "1") {
...
Booking.submit = function (e) {
    ...
    xhReq.onreadystatechange = function() {
        if (xhReq.readyState != 4) return null;
             ...
             if (retvalue != 'True') {
                //response = xhReq.responseText.substring(retvalue.indexOf('\n') + 1); // we jump over the first line which contains doctype like: <!DOCTYPE html>
                if (retvalue.split('\n').length > 1) 
       //Plone 4 if (xhReq.responseText != 'True') {
       //Plone 4     response = xhReq.responseText;
       //Plone 4     if (response.split('\n').length > 1)              
...
function isHTML(testString) {
    var htmlRegex = new RegExp("<(br|basefont|hr|input|source|frame|param|area|meta|!--|col|link|option|base|img|wbr|!DOCTYPE).*?>|<(a|abbr|acronym|address|applet|article|aside|audio|b|bdi|bdo|big|blockquote|body|button|canvas|caption|center|cite|code|colgroup|command|datalist|dd|del|details|dfn|dialog|dir|div|dl|dt|em|embed|fieldset|figcaption|figure|font|footer|form|frameset|head|header|hgroup|h1|h2|h3|h4|h5|h6|html|i|iframe|ins|kbd|keygen|label|legend|li|map|mark|menu|meter|nav|noframes|noscript|object|ol|optgroup|output|p|pre|progress|q|rp|rt|ruby|s|samp|script|section|select|small|span|strike|strong|style|sub|summary|sup|table|tbody|td|textarea|tfoot|th|thead|time|title|tr|track|tt|u|ul|var|video).*?<\/\2>");
